...
public class SecondSpawnLogObjects : MonoBehaviour
{
    [SerializeField]
    public GameObject[] LogObjects;
    [SerializeField]
    SecondGameController gameController;
    [SerializeField]
    ...
    void SpawnLogObject()
    {
        ...
        for (int i = 0; i < EveryObject; i++)
        {
            ...
            if (i < Knives)
            {
                knife = Instantiate(KnifeSpawn, FindObjectOfType<SecondLogRotation>().transform);
